# Meta-Learning on Planning — Implementation Guide

## Overview
After each cycle, log what planning decisions were made and how the cycle performed. Use LLM to reflect on what worked. Feed planning history into future cycle planning prompts.

## New File: `src/orchestrator/planning_memory.py`
...
## Integration into `src/orchestrator/cycle_manager.py`
...
### After each cycle completes in `run_cycle()`:
After the call to `_generate_cycle_report()`, add:
```python
...
Do the same for follow-up cycles in the discovery loop (after `_generate_cycle_report` for follow-up cycles).

### In `_plan_initial_tasks()` prompt:
After the existing `{previous_cycle_context}` section, append:
```python
planning_history = self.planning_memory.get_planning_context(max_outcomes=3)
```

Add `{planning_history}` to the prompt string, after the previous cycle context section.

### In `_plan_synthesis_tasks()` prompt (the Claude-based one):
Same — append `planning_history` to the prompt.

## No changes needed to tree_search.py
Tree search creates its own cycles via the orchestrator, so planning memory automatically gets populated through the existing orchestrator hooks.

## Commit

```bash
git add -A && git commit -m 'feat: meta-learning on planning via outcome tracking and reflection

- New PlanningMemory tracks task plans vs actual results per cycle
- LLM reflects on what worked after each cycle
- Planning history fed into future cycle planning prompts
- Planner learns from failures and successes of previous cycles
- Serializable for persistence via to_dict/from_dict'
```
